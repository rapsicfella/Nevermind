...
function trap_ctrlc ()
{
    ...
    docker stop $OTA_CONTAINER_NAME $VS_CONTAINER_NAME
    docker stop $HVAC_CONTAINER_NAME
...
#docker run -d --rm --net=host --name $VS_CONTAINER_NAME sdv-vs1:latest
...
#	 --name $OTA_CONTAINER_NAME sdv-ota1:latest
...
cd $IVI_UI_FOLDER_PATH/UI_pubSub/ui_pub_sub
python3 ui_sub_integrated.py &
#sleep 1.5

#cd $IVI_UI_FOLDER_PATH/Ice-sdv
#python3 Server_Ice.py &
#sleep 1

#python3 Client_Ice.py &
